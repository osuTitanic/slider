...
class Curve(metaclass=ABCMeta):
    ...
    @lazyval
    def hard_rock(self) -> "Curve":
        """This curve when played with hard rock."""
        req_length = self.req_length
        if req_length is None:
            raise ValueError("Curve.req_length must be set before hard rock")
...
class Bezier(Curve):
    ...
    def __init__(self, points: Sequence[Position], req_length: float | None) -> None:
    ...
    def at(
        self,
        t: float | Sequence[float] | npt.NDArray[np.float64],
        ...
        ixs = np.arange(n + 1)
        return np.sum(
            (comb(n, ixs) * (1 - t_arr) ** (n - ixs) * t_arr**ixs)[:, np.newaxis]
            * self._coordinates,
            axis=-1,
        )
...
class MultiBezier(_MetaCurveMixin, Curve):
    ...
    def __init__(self, points: Sequence[Position], req_length: float) -> None:
        super().__init__(points, req_length)

        self._curves: Sequence[Curve] = [
            Bezier(subpoints, None) for subpoints in self._split_at_dupes(points)
        ]

    @staticmethod
    def _split_at_dupes(input_: Sequence[Position]) -> Iterator[List[Position]]:
...
class Perfect(Curve):
    ...
    def __init__(
        ...
        *,
        ...
        self._center: Position = _center

        coordinates = np.array(points) - np.asarray(_center)
        ...
        self._angle = float(end_angle - start_angle)
...
class Catmull(Curve):
    ...
    def __init__(self, points: Sequence[Position], req_length: float) -> None:
        ...
        self.Cxs: List[npt.NDArray[np.float64]] = []
        self.Cys: List[npt.NDArray[np.float64]] = []
        ...
        p_aheads = np.roll(points_array, 1)
        p_aheads[0] = p_aheads[1]

        p_behinds = np.roll(points_array, -1)
        ...
        for (p1, p2), (t1, t2) in zip(
            zip(points_array, points_array[1:]), zip(tangents_x, tangents_x[1:])
        ):
        ...
        for (p1, p2), (t1, t2) in zip(
            zip(points_array, points_array[1:]), zip(tangents_y, tangents_y[1:])
        ):
    ...
    def __call__(self, t: float) -> Position:
        ...
        # Result of multiplication is a (1,) shaped ndarray so extract scalar.
        px = px.item()
        py = py.item()
        return Position(px, py)


def get_center(a: Position, b: Position, c: Position) -> Position:
    ...
    """
    points = np.array([a, b, c], dtype=np.float64)
    a_vec, b_vec, c_vec = points

    a_squared = np.sum(np.square(b_vec - c_vec))
    b_squared = np.sum(np.square(a_vec - c_vec))
    c_squared = np.sum(np.square(a_vec - b_vec))
    ...
    center = (s * a_vec + t * b_vec + u * c_vec) / sum_
    return Position(float(center[0]), float(center[1]))


def rotate(position: Position, center: Position, radians: float) -> Position:
